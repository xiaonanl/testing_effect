...
type Sim struct {
	...
	TestPats     *etable.Table               `view:"no-inline" desc:"the test patterns to use"`
...
func (ss *Sim) ConfigNet(net *leabra.Network) {
	...
	inp := net.AddLayer2D("Input", 7, 7, emer.Input)
	hid1 := net.AddLayer2D("Hidden1", 10, 10, emer.Hidden)
	out := net.AddLayer2D("Output", 7, 7, emer.Target)
...
func (ss *Sim) ConfigPats() {
	...
	patgen.AddVocabPermutedBinary(ss.PoolVocab, "Train", 6, 7, 7, .2, 0.5)
	patgen.InitPats(ss.TrainPats, "Train", "Train Pats", "Input", "Output", 6, 1, 1, 7, 7)
	patgen.MixPats(ss.TrainPats, ss.PoolVocab, "Input", []string{"Train"})
	patgen.MixPats(ss.TrainPats, ss.PoolVocab, "Output", []string{"Train"})

	testNm := fmt.Sprintf("%dtest", 0)
	trainnoiseNm := fmt.Sprintf("%dNoise", 0)
	tsr, _ := patgen.AddVocabRepeat(ss.PoolVocab, "xxx", 16, "Train", 0)
	patgen.AddVocabRepeat(ss.PoolVocab, "corr", 16, "Train", 0)
	patgen.FlipBitsRows(tsr, 1, 1, 1, 0)

	patgen.VocabSlice(ss.PoolVocab, "xxx", []string{testNm, trainnoiseNm}, []int{0, 6, 16})
	patgen.VocabSlice(ss.PoolVocab, "corr", []string{"testcorr", "traincorr"}, []int{0, 6, 16})

	dt := &etable.Table{}
	patgen.InitPats(dt, trainnoiseNm, trainnoiseNm, "Input", "Output", 10, 1, 1, 7, 7)
	patgen.MixPats(dt, ss.PoolVocab, "Input", []string{trainnoiseNm})
	patgen.MixPats(dt, ss.PoolVocab, "Output", []string{"traincorr"})
	ss.TrainPats.AppendRows(dt)

	patgen.InitPats(ss.TestPats, testNm, testNm, "Input", "Output", 6, 1, 1, 7, 7)
	patgen.MixPats(ss.TestPats, ss.PoolVocab, "Input", []string{testNm})
	patgen.MixPats(ss.TestPats, ss.PoolVocab, "Output", []string{"corr"})

	for i := 1; i < 6; i++ { // rest 5 sets of tests
		testNm := fmt.Sprintf("%dtest", i)
		trainnoiseNm := fmt.Sprintf("%dNoise", i)
		tsr, _ := patgen.AddVocabRepeat(ss.PoolVocab, "xxx", 16, "Train", i)
		patgen.AddVocabRepeat(ss.PoolVocab, "corr", 16, "Train", i)
		patgen.FlipBitsRows(tsr, 1, 1, 1, 0)
		...
		dt := &etable.Table{}
		patgen.InitPats(dt, testNm, testNm, "Input", "Output", 6, 1, 1, 7, 7)
		patgen.MixPats(dt, ss.PoolVocab, "Input", []string{testNm})
		patgen.MixPats(dt, ss.PoolVocab, "Output", []string{"corr"})
		ss.TestPats.AppendRows(dt)
